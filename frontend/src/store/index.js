...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async loadPriceData({ commit, state }) {
      ...
      try {
        console.log('loadPriceData: generating data from', new Date(state.config.timeRange.start), 'to', new Date(state.config.timeRange.end))
        const data = mockData.generatePriceData(
          state.config.timeRange.start,
          state.config.timeRange.end,
          state.config.interval
        )
        console.log('loadPriceData: generated', data.cex?.length || 0, 'data points')
